      ...
      <main className="container mx-auto max-w-7xl px-4 sm:px-6 py-6 flex-grow overflow-x-hidden -mt-8 relative z-20">
        ...
        <div className="grid lg:grid-cols-12 gap-8 md:gap-10 mb-16 items-stretch">
          ...
          <div className="lg:col-span-8 space-y-8 flex flex-col">
            ...
            {announcements.length > 0 && (
              <section className="overflow-hidden w-full flex-grow flex flex-col">
                ...
                <div className="flex-grow">
                    <Swiper
                        modules={[Pagination]}
                        // Mobilde 2 tane, Masaüstünde 3 tane
                        slidesPerView={2} 
                        spaceBetween={12} 
                        breakpoints={{
                    ...
                    {announcements.map((ann) => {
                        ...
                        <SwiperSlide key={ann.id} className="h-auto">
                            {/* Kart tasarımını 'flex-col' (DİKEY) yaptık ki yan yana sığsınlar */}
                            <Link
                                href={`/duyuru/${ann.id}`}
                            ...
                            >
                                {/* Resim Alanı: h-32 (mobil) - h-40 (desktop) */}
                                <div className="h-32 md:h-40 w-full bg-gray-100 relative flex-shrink-0">
                                    ...
                                    />
                                    <div className="absolute top-2 left-2 bg-white/90 backdrop-blur-sm px-2 py-1 rounded-lg text-[9px] font-bold text-gray-600 flex items-center gap-1 shadow-sm z-10">
                                        <FaCalendarAlt size={10} className="text-red-500"/> {new Date(ann.date).toLocaleDateString("tr-TR")}
                                    </div>
                                </div>
                                ...
                                <div className="p-3 flex flex-col flex-grow justify-between">
                                    ...
                                    <div className="flex items-center justify-between mt-2 pt-2 border-t border-gray-50">
                                        <span className="text-[9px] font-bold text-gray-400 uppercase tracking-wide">Duyuru</span>
          ...
          <div className="lg:col-span-4 flex flex-col gap-6 h-full">
            ...
            {/* 2. KUTU: VİDEO (ALT HİZALAMA SAĞLANDI) */}
